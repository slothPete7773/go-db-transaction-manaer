...
func NewRepoUser(q Query) *RepoUser {
	return &RepoUser{q}
}

func (r *RepoUser) WithTx(tx Transaction) *RepoUser {
	return &RepoUser{tx}
}

func (r *RepoUser) CreateUser(ctx context.Context, name string) error {
...
func NewRepoOrder(q Query) *RepoOrder {
	return &RepoOrder{q}
}

func (r *RepoOrder) WithTx(tx Transaction) *RepoOrder {
	return &RepoOrder{tx}
}

func (r *RepoOrder) CreateOrder(ctx context.Context, items []string) error {
	for _, item := range items {
		_, err := r.q.ExecContext(ctx, "INSERT INTO orders (item) VALUES ($1)", item)
		if err != nil {
